...
fig1B_plot <- function(gamma){
  ...
  fig1B <- ggplot(F1B_dat, aes(x=State, y=sr)) + 
    ...
    theme(axis.text.x = element_text(size=18, face="bold", family="Arial"),
          axis.text.y = element_text(size=18, face="bold", family="Arial")) +
    theme(legend.position="None") +
    ylab("Discounted future occupancy \n of the goal state")
...
## Function for plotting figure 2Ba ------------
# Because Figure 2Ba shows the result of a specific single episode, 
# the code for plotting the result may have to be revised depending 
# on the seed used for simulation
fig2Ba_plot <- function(data, gamma) {
  ...
  Action <- c(rep("Go", 9), "Goal", rep("No-Go (Mean)", 2), rep("No-Go (Mean) 2", 2), 
              rep("No-Go (Mean) 3", 3), rep("No-Go (Sum)", 2), rep("No-Go (Sum) 2", 2),
              rep("No-Go (Sum) 3", 3))
  
  # create a dataframe for Fig 2Ba
  F2Ba_dat <- data.frame(State, Action, RPE)
  
  # RPE for cue
  cue_row <- c(0, "Initiation", gamma*data$V1[1])
  F2Ba_dat <- rbind(cue_row, F2Ba_dat)
  F2Ba_dat$State <- as.numeric(F2Ba_dat$State)
  F2Ba_dat$RPE <- as.numeric(F2Ba_dat$RPE)
  F2Ba_dat$Action <- factor(F2Ba_dat$Action, levels=c("Initiation", "Go", "Goal", "No-Go (Mean)", "No-Go (Mean) 2",
                                                      "No-Go (Mean) 3", "No-Go (Sum)", "No-Go (Sum) 2", "No-Go (Sum) 3"))
  
  # Plot
  # prepare a small dataset needed for drawing a line
  jfl <- subset(F2Ba_dat, (State==9 & Action=="Go") | State==10, c(RPE))
  jfl2 <- subset(F2Ba_dat, State==0 | (State==1 & Action=="Go"), c(RPE))
  
  # Plot Figure 2Ba
  fig2Ba <- ggplot(F2Ba_dat, aes(x=State, y=RPE, colour=Action)) +
    geom_point(aes(shape=Action, colour=Action), size=4, stroke=1.5) +
    geom_hline(yintercept=0, linetype="dashed", colour="gray") +
    geom_line(size=1, aes(linetype=Action, colour=Action)) +
    scale_colour_manual(values=c("brown", "blue", "black", "red", "red", "red",
                                 "magenta", "magenta", "magenta")) +
    scale_shape_manual(values=c(4,4,4,4,4,4,1,1,1)) +
    scale_linetype_manual(values=c("solid", "solid", "solid", "dashed", "dashed", 
                                   "dashed", "dashed", "dashed", "dashed")) +
    scale_x_continuous(breaks=seq(0,10,1)) + 
    scale_y_continuous(breaks=c(-0.25, 0.0, 0.25, 0.5, 0.75), expand=c(0,0)) +
    ...
    geom_segment(aes(x=0, y=jfl2[1,1], xend=1, yend=jfl2[2,1]), linetype="solid", colour="brown") +
    coord_cartesian(xlim=c(0,10), ylim=c(-0.25, 0.80)) +
    theme_classic() +
    theme(aspect.ratio=5/6) +
    theme(axis.title.x = element_text(size=20, family="Arial", face="bold"),
          axis.title.y = element_text(size=20, family="Arial", face="bold")) + 
...
## ------ Function for plotting figure 3Ba -------
# Because Figure 3Ba shows the result of a specific single episode, 
# the code for plotting the result may have to be revised depending 
# on the seed used for simulation

fig3Ba_plot <- function(data, gamma) {
  ...
  # create a dataframe for Fig 3Ba
  F3Ba_dat <- data.frame(State, Action, RPE)
  
  # RPE for cue
  cue_row <- c(0, "Initiation", gamma*data$V1[1])
  F3Ba_dat <- rbind(cue_row, F3Ba_dat)
  F3Ba_dat$State <- as.numeric(F3Ba_dat$State)
  F3Ba_dat$RPE <- as.numeric(F3Ba_dat$RPE)
  F3Ba_dat$Action <- factor(F3Ba_dat$Action, levels=c("Initiation", "Go", "Goal", "No-Go (Mean)", "No-Go (Mean) 2",
                                                      "No-Go (Sum)", "No-Go (Sum) 2"))
  
  # Plot
  # prepare a small dataset needed for drawing a line
  jfl <- subset(F3Ba_dat, (State==9 & Action=="Go") | State==10, c(RPE))
  jfl2 <- subset(F3Ba_dat, State==0 | (State==1 & Action=="Go"), c(RPE))
  
  # Plot Figure 3Ba
  fig3Ba <- ggplot(F3Ba_dat, aes(x=State, y=RPE, colour=Action)) +
    geom_point(aes(shape=Action, colour=Action), size=4, stroke=1.5) +
    geom_hline(yintercept=0, linetype="dashed", colour="gray") +
    geom_line(size=1, aes(linetype=Action, colour=Action)) +
    scale_colour_manual(values=c("brown", "blue", "black", "red", "red", "magenta", "magenta")) +
    scale_shape_manual(values=c(4,4,4,4,4,1,1)) +
    scale_linetype_manual(values=c("solid", "solid", "solid", "dashed", "dashed", "dashed", "dashed")) +
    scale_x_continuous(breaks=seq(0,10,1)) + 
    scale_y_continuous(breaks=c(-0.25, 0.0, 0.25, 0.5, 0.75), expand=c(0,0)) +
    # Draw a line between RPE at state 8 and state 9
    geom_segment(aes(x=9, y=jfl[1,1], xend=10, yend=jfl[2,1]), linetype="solid", colour="black") +
    geom_segment(aes(x=0, y=jfl2[1,1], xend=1, yend=jfl2[2,1]), linetype="solid", colour="brown") +
    coord_cartesian(xlim=c(0,10), ylim=c(-0.25, 0.8)) +
    theme_classic() +
    theme(aspect.ratio=5/6) +
    theme(axis.title.x = element_text(size=20, family="Arial", face="bold"),
          axis.title.y = element_text(size=20, family="Arial", face="bold")) + 
...
hState_vRPE_plot <- function(data, gamma, epi_num, fig_num, legend) {
  ...
  # dataframe for cue RPE
  crpe <- filter(data, State==1 & time_step==1) %>%
    mutate(cue_RPE = gamma*V1) %>%
    select(Simulation, Episode, cue_RPE) %>%
    group_by(Episode) %>%
    summarize(cue_RPE_mean=mean(cue_RPE), cue_RPE_sd=sd(cue_RPE), .groups="drop")
  
  # Add RPE at the onset
  cue_row <- c(0, "Initiation", as.numeric(crpe[epi_num, 2]), as.numeric(crpe[epi_num, 3]))
  fig_dat <- rbind(cue_row, fig_dat)
  fig_dat$State <- as.numeric(fig_dat$State)
  fig_dat$RPE_mean <- as.numeric(fig_dat$RPE_mean)
  fig_dat$RPE_sd <- as.numeric(fig_dat$RPE_sd)
  fig_dat$Action <- factor(fig_dat$Action, levels=c("Initiation", "Go", "Goal", "No-Go (Mean)", "No-Go (Sum)"))
  
  # Plot
  # prepare a small dataset needed for drawing a line
  jfl <- subset(fig_dat, (State==9 & Action=="Go") | State==10, c(RPE_mean))
  jfl2 <- subset(fig_dat, State==0|(State==1 & Action=="Go"), c(RPE_mean))
  ...
  if (fig_num != "S1") {
    if (legend == 1) { # with legend
      figure <- ggplot(fig_dat, aes(x=State, y=RPE_mean, colour=Action)) +
        ...
        geom_point(aes(shape=Action, colour=Action), size=4, stroke=1.5) +
        geom_line(size=1, aes(linetype=Action)) +
        # Draw a line between RPE at state 9 and state 10
        geom_segment(aes(x=9, y=jfl[1,1], xend=10, yend=jfl[2,1]), 
                     linetype="solid", colour="black") +
        geom_segment(aes(x=0, y=jfl2[1,1], xend=1, yend=jfl2[2,1]),
                     linetype="solid", colour="brown") + 
        scale_colour_manual(values=c("brown", "blue", "black", "red", "magenta")) +
        scale_shape_manual(values=c(4,4,4,4,1)) +
        scale_linetype_manual(values=c("solid", "solid", "solid", "dashed", "dashed")) +
        scale_x_continuous(breaks=seq(0,10,1)) + 
        scale_y_continuous(breaks=c(-0.25, 0.0, 0.25, 0.5, 0.75), expand=c(0,0)) +
        coord_cartesian(xlim=c(0,10), ylim=c(-0.25, 0.80)) +
  ...
  # Supplemental figures S1
    ...
    figure <- ggplot(fig_dat, aes(x=State, y=RPE_mean, colour=Action)) +
      ...
      scale_shape_manual(values=c(4,4,4,4,1)) +
      scale_linetype_manual(values=c("solid", "solid", "solid", "dashed", "dashed")) +
      scale_x_continuous(breaks=seq(0,10,1), labels=c("0", "", "", "", "", "", "", "", "", "", "10")) + 
      scale_y_continuous(breaks=seq(-0.8, 1.0, 0.2), 
                         labels=c("-0.8", "" , "", "", "0.0", "", "", "", "", "1.0"),
                         expand = c(0.0,0.0)) +
      coord_cartesian(xlim=c(0,10), ylim=c(-0.8,1.0)) +
      theme_classic() +
      theme(aspect.ratio=5/6) +
      theme(axis.title.x = element_text(size=21, family="Arial", face="bold"),
            axis.title.y = element_text(size=21, family="Arial", face="bold")) + 
  ...
  if (fig_num != "S3") {
    if (legend == 1) { # with legend
      figure <- ggplot(fig_dat, aes(x=Episode, y=RPE_mean, group=interaction(State, Action))) +
        ...
        coord_cartesian(xlim=c(0,200), ylim=c(-0.25, 0.55)) + 
        theme_classic() +
        theme(aspect.ratio=5/6) +
        theme(axis.title.x = element_text(size=20, family="Arial", face="bold"),
              axis.title.y = element_text(size=20, family="Arial", face="bold")) + 
...
## -------- Function for visualizing change of cue RPE ----------
hEpisode_vcue_plot <- function(data, gamma, epi_num, fig_num) {
  
  d <- data %>%
    filter(time_step==1 & Episode < epi_num+1) %>%
    mutate(cue_RPE=gamma*V1) %>%
    select(Simulation, Episode, cue_RPE) %>%
    group_by(Episode) %>%
    summarize(mean=mean(cue_RPE), sd=sd(cue_RPE), .groups="drop")
  
  if (fig_num != "S2"){
    figure <- ggplot(d, aes(x=Episode, y=mean)) +
      #geom_hline(yintercept=0, linetype="dashed", colour="gray") +
      geom_line(colour="brown", size=0.70) +
      geom_ribbon(aes(ymin=mean-sd, ymax=mean+sd), 
                  fill="brown", alpha=0.2) +
      scale_y_continuous(breaks=c(0.0, 0.25, 0.5, 0.75), expand=c(0,0)) + 
      coord_cartesian(xlim=c(0,epi_num), ylim=c(0.0, 0.80)) + 
      theme_classic() +
      theme(aspect.ratio=5/6) +
      theme(axis.title.x = element_text(size=20, family="Arial", face="bold"),
            axis.title.y = element_text(size=20, family="Arial", face="bold")) + 
    ...
    figure <- ggplot(d, aes(x=Episode, y=mean)) +
      ...
      geom_ribbon(aes(ymin=mean-sd, ymax=mean+sd), 
                  fill="brown", alpha=0.2) +
      scale_y_continuous(breaks=c(0.0, 0.5, 1.00), expand=c(0,0)) + 
      coord_cartesian(xlim=c(0,epi_num), ylim=c(0.0, 1.00)) + 
      theme_classic() +
      theme(aspect.ratio=5/6) +
      theme(axis.title.x = element_text(size=21, family="Arial", face="bold"),
            axis.title.y = element_text(size=21, family="Arial", face="bold")) + 
...
fig5A_plot <- function(data, epi1, epi2, epi3, legend) {
  ...
  if (legend == 1) { # with legend
    fig5A <- ggplot(F5A_dat, aes(x=State, y=sr_m, colour=Episode)) +
      ...
      geom_point(aes(shape=Episode, colour=Episode), size=3, stroke=1.0) +
...
## -------- Function for plotting Figure 6Ca and 6Cb ---------
hState_vCoef_plot <- function(data, epi_num) {
  
  w_dat <- data %>%
    filter(Episode == epi_num & State == 10) %>%
    select(-X, -time_step, -V1, -Episode, -State, -Action, -RPE) %>%
    summarize_at(vars(W1:W10), funs(mean, sd))
  ...
  figure <- ggplot(coef_dat, aes(x=w_index, y=w_mean)) +
    ...
    geom_point(size=4, stroke=1.0, shape=1) +
    geom_line() +
...
## ------- Function for plotting Figure 6D --------
fig6D_plot <- function(data, epi_num, legend) {
  ...
  F6D_dat <- data.frame(Episode, w_index, w_mean, w_sd)
  F6D_dat$w_index <- as.factor(F6D_dat$w_index)
  ...
  if (legend == 1) {# with legend
    # plot Figure 6D
    fig6D <- ggplot(F6D_dat, aes(x=Episode, y=w_mean, group=w_index)) +
  ...
  return(list(fig6D, fig6D_legend))
}
  ...
  if (legend == 1) {
    figure <- ggplot(qdat2, aes(x=State, y=Q_mean, group=Action)) +
      geom_errorbar(aes(ymax=Q_mean+Q_sd, ymin=Q_mean-Q_sd, colour=Action), 
                    width=0.4) + 
      geom_point(aes(colour=Action, shape=Action), size=4, stroke=1.5) + 
...
hState_vQdif_plot <- function(data, epi1, epi2) {
  ...
  figure <- ggplot(qdif_dat, aes(x=State, y=Qdif_mean2)) +
    geom_point(colour="turquoise", size=4, stroke=1.5) +
...
hState_vQdif_plot2 <- function(kappa0, kappa20, kappa40, epi1, epi2, legend) {
  ...
  if (legend == 1){# with legend
    figure <- ggplot(fig_dat, aes(x=State, y=Qdif_mean, group=Kappa)) +
      ...
      geom_point(aes(fill=Kappa), shape=21, size=4, stroke=1.0) +
