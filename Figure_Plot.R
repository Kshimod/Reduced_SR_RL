...
ab_path = "C:/Users/shimo/Desktop/Add_simulation3"
setwd(ab_path)
...
v <- w*feat # Approximated state values (equal to v_true when w=1)
...
for (p_stay in c(0.50, 0.75, 0.90)) {
  ...
  mean <- append(mean, c(mean(redu$cue_rpe), mean(punc$cue_rpe)))
  sd <- append(sd, c(sd(redu$cue_rpe), sd(punc$cue_rpe)))
}

# with Punishment
puni <- read.csv("Punctate/NonR_punish_g97_s0_11states_r1.0_p2.0.csv") %>%
  cue_ex(gamma, epi_num=25)
mean <- append(mean, c(0.97**10, mean(puni$cue_rpe)))
sd <- append(sd, c(0.0, 0.0))


cond <- c(rep(1, 2), rep(2, 2), rep(3, 2),
          rep(4, 2), rep(6, 2))

rl_type <- c(rep(c("Rigid reduced SR", "Simple RL"), 4), "Theoritical value with rigid reduced SR", "Simple RL")

d <- data.frame(cond, rl_type, mean, sd)
d$rl_type <- factor(d$rl_type, levels=c("Simple RL", "Rigid reduced SR", "Theoritical value with rigid reduced SR"))


# plot
p <- ggplot(d, aes(x=cond, y=mean, fill=rl_type)) +
  geom_hline(yintercept=0, color="gray", linetype="dashed") +
  geom_col_pattern(aes(cond, mean, pattern_fill= rl_type, pattern=rl_type, 
                       fill=rl_type, colour=rl_type, linetype=rl_type, pattern_size=rl_type), 
                   position="dodge", pattern_alpha=0.8, pattern_colour="red", 
                   pattern_density=0.4, pattern_spacing=0.02, width=0.8) +
  geom_segment(aes(x=1, y=0.97**10, xend=6, yend=0.97**10),
  ...
  scale_y_continuous(breaks=c(seq(-0.25, 0.75, 0.25))) +
  scale_x_continuous(breaks=c(1,2,3,4,6), labels=c("Non-Resistant", "P(No-Go) = 0.50", "P(No-Go) = 0.75", 
                                                   "P(No-Go) = 0.90", "Non-Resistant \n with punishment")) +
  scale_pattern_size_manual(values=c(0,0,0.1)) +
  scale_pattern_fill_manual(values=c("black", "black", "red")) +
...
# save
ggsave("fig6B.jpg", plot=fig6B[[1]], path=fig6_path)
...
fig_num <- "S4"
p <- hEpisode_vRPE_plot(data, epi_num, fig_num, legend)
ggsave("figS4C.jpg", plot=p[[1]], path=figS_path)
